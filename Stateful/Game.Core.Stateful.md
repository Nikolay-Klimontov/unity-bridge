...
Набор классов для создания подсистем/сервисов, логика работы которых меняется в зависимости от их текущего состояния (например GameLoop)
...
Если автомат работает на общей [шине](../Messaging/Game.Core.Messaging.md), то было бы неплохо его на эту шину подписать. Подписаться на получение можно в любой момент
